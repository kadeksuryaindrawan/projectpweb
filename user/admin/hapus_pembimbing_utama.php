<?php  
	...
	if(isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])){
		...
		if(mysqli_num_rows($query_check) == 1){
			$query_delete= mysqli_query($connection, "DELETE FROM pembimbing_utama WHERE id = $id");
			if($query_delete){
				$pesan = "Pembimbing Utama sukses dihapus!";
			}
			else{
				$pesan = "Pembimbing Utama gagal dihapus!";
			}
			echo "	<script>
		 				alert('$pesan');

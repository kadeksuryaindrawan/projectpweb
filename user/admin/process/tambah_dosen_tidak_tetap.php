<?php  
	...
	if(isset($_POST['submit'])){
		...
		if(!empty($email) && !empty($password) && !empty($repassword) && !empty($nip) && !empty($nidn) && !empty($nama_dosen) && !empty($alamat) && !empty($telp) && !empty($prodi) && !empty($pendidikan_terakhir) && !empty($bidang_keahlian) && !empty($DTPS) && !empty($jabatan_akademik) && !empty($nomor_serdos) && !empty($kesesuaian_bidang) && !empty($status_dosen)){
			if($password == $repassword){
				if(strlen($password) >= 7 ){
					...
					if(mysqli_num_rows($query_check) == 0){
                        ...
                        $query_insert2 = mysqli_query($connection, "INSERT INTO dosen VALUES ('$nip','$nidn','$nama_dosen','$alamat','$telp', $user_id, $prodi,'$pendidikan_terakhir','$bidang_keahlian','$DTPS',$jabatan_akademik,'$nomor_serdos','$kesesuaian_bidang','$status_dosen')");

<?php  
	...
	if(isset($_POST['submit'])){
		extract($_POST);
		if(!empty($email) && !empty($nip_edit) && !empty($nidn) && !empty($nama_dosen) && !empty($alamat) && !empty($telp) && !empty($prodi) && is_numeric($nip)){
                if($password == ""){
                    $query_update_user = mysqli_query($connection, "UPDATE users SET email = '$email' WHERE user_id = $user_id");
                    $query_update_dosen = mysqli_query($connection, "UPDATE dosen SET nip = '$nip_edit', nidn = '$nidn', nama_dosen = '$nama_dosen', alamat = '$alamat', no_telp = '$telp', prodi = $prodi WHERE nip = '$nip'");

			...
			if($password == $repassword){
				if(strlen($password) >= 7 ){
					...
					if(mysqli_num_rows($query_check) == 0){
						...
						$query_insert = mysqli_query($connection, "INSERT INTO users VALUES (NULL, '$email', '$encrypted_password', 'guest')");
						if($query_insert){
								echo"
										<script>
											alert('Sukses Tambah Guest');
											location.href = '../tambah_guest.php';
										</script>
									";
						}
						else{
